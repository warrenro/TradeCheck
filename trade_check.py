...
class TradeAuditor:
    ...
    def load_transactions_from_csv(self, file_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            required_columns = {'成交時間', '買賣別', '平倉損益淨額', '口數', '商品名稱', '新倉價', '平倉價', '手續費', '期交稅'}
            ...
            df['fee'] = pd.to_numeric(df['fee'].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
            df['tax'] = pd.to_numeric(df['tax'].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
