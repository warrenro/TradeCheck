...
為了提供更精準的交易分析與風險評估，系統近期已整合以下新功能並修正了數個關鍵錯誤：

- **圖表功能優化與修復**: 修正了交易圖層因資料不完整而無法顯示的錯誤，並解決了技術指標導致Y軸縮放異常的問題，大幅提升圖表穩定性。
- **穩定性修復**: 修正了數個導致伺服器啟動失敗以及 K 線圖在切換時間週期時無法顯示的嚴重錯誤。
...
- **帳戶權益數動態計算**: 帳戶的「目前權益數」現在會基於 `config.ini` 中的 `monthly_start_capital` 加上當月的損益 (Monthly PnL) 動態計算。
...
- **K線圖顯示功能 (Candlestick Chart Display)**: 
  ...
  - **新增功能工具列**，提供以下進階分析功能：
    ...
    - **技術指標圖層**: 允許使用者在圖表上疊加移動平均線 (MA) 與 布林通道 (Bollinger Bands)。
